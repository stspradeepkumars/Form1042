...
*** Settings ***
Resource   ../../support/1042ero_support.robot
...
Login to Account
  ...
  ${siteURL}=    	set Variable    https://tbs-secure.spanuat.com/login
  ${uid}=    	set Variable    puvanasri.j+1042EROUAT@spantechnologyservices.com  
  ...
  Run Keyword If	${passed}    Click Element 	 //button[contains(@class,"text-capitalize")]/span[contains(text(),"Start Filling")]
  Remove Files   ../../Test suite/Form1042/Screenshot/*.png
  SelectNewForm1042  ${start}    ${form}  ${1042}
  sleep  15s
  ${passed} =    Run Keyword And Return Status         Wait Until Element Is Enabled       (//span[@class="v-btn__content"])[28]    100s
	Run Keyword If	${passed}    Click Element    (//span[@class="v-btn__content"])[28]
  sleep	10s
